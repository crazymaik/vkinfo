...
fn enumerate_extension_for_layer(functions: &vulkan::Functions, layer_name: *const c_char) -> vulkan::Result<()> {
...
fn main() {
    ...
    let functions = vulkan::Functions::new().unwrap();

    println!("Enumerating layers");

    let layers = functions.enumerate_instance_layer_properties().unwrap();

    println!("Found {0} layers: ", layers.len());

    enumerate_extension_for_layer(&functions, ptr::null()).unwrap();
    ...
    for layer in &layers {
        unsafe { 
            ...
            enumerate_extension_for_layer(&functions, layer.layer_name.as_ptr()).unwrap();
        }
    }
}
